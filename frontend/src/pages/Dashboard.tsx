...
const Dashboard: React.FC = () => {
    ...
    // Check-in reminder for interns
    useEffect(() => {
        if (user?.role === 'INTERN') {
            const hasSeenReminder = sessionStorage.getItem('attendance_reminder_shown');
            if (!hasSeenReminder) {
                setTimeout(() => {
                    toast('Remember to Check In when you start, and Check Out when you leave!', {
                        icon: '⏰',
                        duration: 6000,
                        style: {
                ...
                }, 1000); // Small delay to let the dashboard render first
                sessionStorage.setItem('attendance_reminder_shown', 'true');
            }
        }
    }, [user]);
    ...
    if (user?.role === 'ADMIN') {
        statItems = [
            { label: 'Total Admins', value: stats.total_managers ?? '...', icon: <Users size={24} />, color: 'from-pink-500 to-rose-500', trend: '+12%', trendUp: true },
