...
import { AttendanceHeatmap, Card, CustomSelect } from '../components/common';
...
const MyAttendance: React.FC = () => {
    ...
    const handleAttendanceAction = async (action: 'check-in' | 'check-out', targetDate?: string) => {
        if (user?.role !== 'INTERN') {
            toast.error('Only interns can mark attendance');
            return;
        }
        ...
        const existingRecord = allAttendance.find(a => a.date === dateToUse);
        ...
        if (action === 'check-in') {
            ...
            if (existingRecord?.check_out_time) {
                payload.check_out_time = existingRecord.check_out_time;
            }
        } else {
            if (existingRecord?.check_out_time) {
            ...
            if (existingRecord?.check_in_time) {
                payload.check_in_time = existingRecord.check_in_time;
            } else {
        ...
        try {
            await api.post('/analytics/attendance/mark/', payload);
            toast.success(`Successfully marked ${action.replace('-', ' ')} for today`);
            fetchMyAttendance();
            fetchHeatmapData();
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        } catch (error: any) {
            const errMessage = error.response?.data?.error || `Failed to ${action.replace('-', ' ')}`;
                    ...
                    {user?.role === 'INTERN' && (
                        <div className="flex gap-3">
                            <button
                                ...
                                className="px-6 py-2.5 bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 rounded-xl hover:bg-emerald-500/30 hover:border-emerald-500/50 transition-all font-medium flex items-center gap-2"
                            >
                                <CheckCircle size={18} />
                                Check In
                            </button>
                            <button
                                ...
                                className="px-6 py-2.5 bg-orange-500/20 text-orange-400 border border-orange-500/30 rounded-xl hover:bg-orange-500/30 hover:border-orange-500/50 transition-all font-medium flex items-center gap-2"
                            >
                                <Clock size={18} />
                                Check Out
                            </button>
