...
const AITaskGenerator: React.FC = () => {
    ...
    // Editing state
    const [editingIndices, setEditingIndices] = useState<{pIdx: number, tIdx: number} | null>(null);
    ...
    useEffect(() => {
        if (selectedIntern) {
            const saved = localStorage.getItem(`aims_pathway_history_${selectedIntern}`);
            if (saved) {
                try {
            ...
            success: (data) => {
                ...
                if (data.pathways && data.pathways.length > 0) {
                    setPathwayHistory(prev => {
                        const newEntry = { timestamp: new Date().toLocaleString(), pathways: data.pathways };
                        const newHistory = [newEntry, ...prev].slice(0, 10); // Keep last 10
                        if (selectedIntern) {
                            localStorage.setItem(`aims_pathway_history_${selectedIntern}`, JSON.stringify(newHistory));
                        }
                        return newHistory;
                    });
                }

                if (!data.pathways || data.pathways.length === 0) return "Synthesis complete, but no suggestions emerged from context.";
                return "Task pathways successfully synthesized";
            },
            error: (err) => {
                setGenerating(false);
    ...
    const assignTask = async (task: AITaskSuggestion, pIdx: number, tIdx: number) => {
        if (!selectedIntern) return;
        
        const isEditing = editingIndices?.pIdx === pIdx && editingIndices?.tIdx === tIdx;
        const finalTask = {
            ...task,
            title: isEditing ? editValues.title : task.title,
            description: isEditing ? editValues.description : task.description
        };
        ...
        toast.promise(axios.post('/analytics/tasks/create/', {
            ...
            success: () => {
                ...
                newPathways[pIdx].tasks = newPathways[pIdx].tasks.filter((_, i) => i !== tIdx);
                if (newPathways[pIdx].tasks.length === 0) {
                    newPathways.splice(pIdx, 1);
                }
                setAiPathways(newPathways);
                
                if (editingIndices?.pIdx === pIdx && editingIndices?.tIdx === tIdx) setEditingIndices(null);
                setAssigning(null);
    ...
    const startEditing = (pIdx: number, tIdx: number) => {
        setEditingIndices({ pIdx, tIdx });
        setEditValues({
            title: aiPathways[pIdx].tasks[tIdx].title,
            description: aiPathways[pIdx].tasks[tIdx].description
        });
    };

    const saveEdit = (pIdx: number, tIdx: number) => {
        const updatedPathways = [...aiPathways];
        updatedPathways[pIdx].tasks[tIdx] = {
            ...updatedPathways[pIdx].tasks[tIdx],
            title: editValues.title,
            description: editValues.description
        };
        setAiPathways(updatedPathways);
    ...
    const assignPathway = async (pIdx: number) => {
        ...
        toast.promise(
            Promise.all(pathway.tasks.map(task => 
                ...
                loading: `Transmitting Pathway: ${pathway.name}...`,
                success: () => {
                    const newPathways = [...aiPathways];
                    newPathways.splice(pIdx, 1);
                    setAiPathways(newPathways);
                    setAssigningPathway(null);
                    return `Pathway '${pathway.name}' fully anchored`;
                },
                error: (err) => {
                    setAssigningPathway(null);
                    ...
                    <main className="bg-[var(--bg-color)] relative overflow-y-auto custom-scrollbar p-10 flex flex-col items-center">
                        ...
                        <div className="max-w-4xl w-full flex flex-col h-full">
                            ...
                            <div className="flex items-center justify-between mb-10 w-full animate-fade-in">
                                ...
                                {aiPathways.length > 0 && <span className="bg-[var(--bg-muted)] border border-[var(--border-color)] px-4 py-1.5 rounded-full text-xs font-bold text-purple-600 dark:text-purple-400 font-mono tracking-tighter uppercase">{aiPathways.length} Pathways</span>}
                            </div>

                            {!generating && aiPathways.length === 0 && (
                            ...
                            {/* Suggestions List */}
                            <div className="space-y-12 pb-20 w-full animate-fade-in">
                                {aiPathways.map((pathway, pIdx) => (
                                    <div key={pIdx} className="space-y-6">
                                        <div className="bg-purple-500/5 border border-purple-500/20 rounded-3xl p-8 mb-6 shadow-[0_0_50px_rgba(168,85,247,0.05)] flex items-start justify-between gap-6">
                                            <div>
                                                <h3 className="text-2xl font-black text-purple-500 mb-3">{pathway.name}</h3>
                                                <p className="text-[var(--text-dim)] font-medium leading-relaxed max-w-3xl">{pathway.description}</p>
                                            </div>
                                            <button 
                                                onClick={() => assignPathway(pIdx)}
                                                disabled={assigningPathway === pIdx}
                                                className="shrink-0 flex items-center justify-center gap-2 px-6 py-3 bg-purple-600 text-white font-black uppercase tracking-widest text-xs rounded-xl hover:bg-purple-500 transition-all duration-300 shadow-lg shadow-purple-500/20 active:scale-95 disabled:opacity-50"
                                            >
                                                {assigningPathway === pIdx ? <Loader2 size={16} className="animate-spin" /> : <Layers size={16} />}
                                                {assigningPathway === pIdx ? 'ASSIGNING...' : 'ASSIGN FULL PATHWAY'}
                                            </button>
                                        </div>
                                        
                                        <div className="space-y-8 pl-8 border-l-4 border-purple-500/20">
                                                    ...
                                                    <div className="p-10">
                                                        <div className="flex items-start justify-between gap-6 mb-8">
                                                            <div className="space-y-4 flex-1">
                                                                ...
                                                                {editingIndices?.pIdx === pIdx && editingIndices?.tIdx === tIdx ? (
                                                                    <input 
                                                            ...
                                                            <div className="flex flex-col gap-2">
                                                                ...
                                                                {editingIndices?.pIdx === pIdx && editingIndices?.tIdx === tIdx ? (
                                                                    <div className="flex gap-2">
                                                                        ...
                                                                        <button 
                                                                            onClick={() => setEditingIndices(null)}
                                                                    ...
                                                                    <button 
                                                                        onClick={() => startEditing(pIdx, tIdx)}
                            ...
                            {/* Pathway History Section */}
                            {pathwayHistory.length > 0 && (
                                <div className="mt-10 pt-10 border-t border-[var(--border-color)] space-y-6 animate-fade-in w-full">
                                    <div className="flex items-center gap-4 mb-4">
                                        <h3 className="text-xs font-bold uppercase tracking-[0.3em] text-[var(--text-muted)] flex items-center gap-3">
                                            <History size={16} /> Previously Generated Pathways
                                        </h3>
                                        <div className="flex-1 h-px bg-[var(--border-color)]" />
                                    </div>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        {pathwayHistory.map((historyItem, idx) => (
                                            <div key={idx} className="p-6 rounded-[32px] bg-[var(--card-bg)] border border-[var(--border-color)] space-y-4 hover:border-purple-500/30 transition-all group">
                                                <div className="flex items-center justify-between px-2">
                                                    <span className="text-[10px] font-black uppercase tracking-widest text-[var(--text-muted)]">{historyItem.timestamp}</span>
                                                    <Badge variant="default" size="sm">{historyItem.pathways.length} Pathways</Badge>
                                                </div>
                                                <div className="space-y-3">
                                                    {historyItem.pathways.map((p, i) => (
                                                        <div key={i} className="flex items-center justify-between p-4 rounded-2xl bg-[var(--bg-color)] border border-[var(--border-color)]">
                                                            <p className="text-sm font-bold text-[var(--text-dim)] truncate">{p.name}</p>
                                                            <span className="text-[10px] font-mono text-[var(--text-muted)] bg-[var(--bg-muted)] px-2 py-1 rounded-md">{p.tasks.length} Tasks</span>
                                                        </div>
                                                    ))}
                                                </div>
                                                <button 
                                                    onClick={() => setAiPathways(historyItem.pathways)}
                                                    className="w-full py-3 mt-4 bg-[var(--bg-muted)] text-[var(--text-main)] font-bold uppercase tracking-widest text-[10px] rounded-xl hover:bg-purple-600 hover:text-white transition-all duration-300 opacity-0 group-hover:opacity-100 translate-y-2 group-hover:translate-y-0"
                                                >
