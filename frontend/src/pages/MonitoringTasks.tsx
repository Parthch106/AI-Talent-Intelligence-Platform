...
import { Card, ContributionHeatmap, Modal, Button, CustomSelect } from '../components/common';
...
const MonitoringTasksPage: React.FC = () => {
    ...
    const getSelectedInternName = (): string => {
        ...
        if (selectedIntern === null) return 'All Interns';
        const selected = interns.find(i => i.id === selectedIntern);
        return selected?.full_name || selected?.email || 'All Interns';
    };
                        ...
                        <div className="flex items-center gap-3 overflow-visible">
                            ...
                            <div className="relative z-30">
                                <button
                                    ...
                                    className="flex items-center gap-3 px-4 py-2.5 bg-[var(--bg-muted)] border border-[var(--border-color)] rounded-xl hover:border-purple-500/50 transition-all font-bold text-xs uppercase tracking-widest text-[var(--text-dim)] shadow-lg backdrop-blur-md"
                                >
                                ...
                                {showStatusDropdown && (
                                    <div className="absolute top-full right-0 mt-2 w-48 bg-[var(--bg-color)] backdrop-blur-3xl border border-[var(--border-color)] rounded-xl shadow-2xl z-[9999] animate-scale-in">
                                        <div className="p-1">
                            ...
                            <div className="relative z-30">
                                <button
                                    ...
                                    className="flex items-center gap-3 px-4 py-2.5 bg-[var(--bg-muted)] border border-[var(--border-color)] rounded-xl hover:border-purple-500/50 transition-all shadow-lg backdrop-blur-md"
                                >
                                    <div className={`w-8 h-8 rounded-lg bg-gradient-to-br ${getGradient(getSelectedInternName())} flex items-center justify-center text-[var(--text-main)] font-bold text-xs`}>
                                        {selectedIntern ? getInitials(interns.find(i => i.id === selectedIntern)?.full_name || null) : 'ALL'}
                                    </div>
                                ...
                                {showInternDropdown && (
                                    <div className="absolute top-full right-0 mt-2 w-[350px] bg-[var(--bg-color)] backdrop-blur-3xl border border-[var(--border-color)] rounded-xl shadow-2xl z-[9999] isolate animate-scale-in max-h-[400px] overflow-y-auto custom-scrollbar">
                                        <div className="p-2">
                                            ...
                                            {!internSearch && (
                                                <button
                                                    ...
                                                    className={`w-full flex items-center gap-3 p-2 rounded-lg transition-colors ${selectedIntern === null ? 'bg-purple-500/20' : 'hover:bg-[var(--bg-muted)]'}`}
                                                >
                                                    <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-purple-500 to-indigo-500 flex items-center justify-center text-[var(--text-main)] font-bold text-xs">
                                                        ALL
                                                    </div>
                                                    <div className="text-left">
                                                        <p className="text-[var(--text-main)] text-sm font-bold">All Interns</p>
                                                        <p className="text-xs text-[var(--text-muted)]">View all tasks</p>
                                                    </div>
                                                </button>
                                            )}
                ...
                {loading ? (
                    ...
                    <>
                        ...
                        <TasksTab
                            ...
                            canCreate={user?.role === 'MANAGER' && !!selectedIntern}
                ...
                <form onSubmit={handleCreateTask} className="space-y-5">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <CustomSelect
                                options={[
                                    { value: '', label: selectedIntern ? "Select Project (Intern's Projects Only)" : 'No Project' },
                                    ...projects.filter((p) => {
                                        if (!selectedIntern) return true;
                                        const internId = typeof p.intern === 'object' ? p.intern?.id : p.intern;
                                        return internId === selectedIntern;
                                    }).map((p) => ({ value: String(p.id), label: p.project?.name || '' }))
                                ]}
                                value={String(taskForm.project_assignment_id)}
                                onChange={(v) => {
                                    const selectedProj = projects.find((p) => String(p.id) === v);
                                    const projectId = selectedProj?.project?.id;
                                    setTaskForm({ ...taskForm, project_assignment_id: v, project_module_id: '' });
                        ...
                        <div>
                            ...
                            <CustomSelect
                                options={[
                                    { value: '', label: taskForm.project_assignment_id ? 'Select Module' : 'No project selected' },
                                    ...modules.map((m) => ({ value: String(m.id), label: m.name }))
                                ]}
                                value={String(taskForm.project_module_id)}
                                onChange={(v) => setTaskForm({ ...taskForm, project_module_id: v })}
