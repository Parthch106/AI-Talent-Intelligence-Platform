...
const Login: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!otpRequired) {
            ...
            toast.promise(requestOtpPromise(), {
                loading: 'Authenticating credentials with secure core...',
                success: (data) => {
                    setIsLoading(false);
                ...
                error: (err) => {
                    setIsLoading(false);
                    const apiError = err as { response?: { data?: { detail?: string, non_field_errors?: string[] } } };
                    const msg = apiError.response?.data?.detail || apiError.response?.data?.non_field_errors?.[0] || 'Authentication sequence failed';
                    setError(msg);
                    return msg;
                }
            });
            ...
            toast.promise(verifyOtpPromise(), {
                loading: 'Verifying secure access code...',
                success: (user) => {
                    navigate('/');
                    setIsLoading(false);
                    return `Access granted. Welcome back, ${user.full_name}`;
                ...
                error: (err) => {
                    setIsLoading(false);
                    const apiError = err as { response?: { data?: { detail?: string, non_field_errors?: string[] } } };
                    const msg = apiError.response?.data?.detail || apiError.response?.data?.non_field_errors?.[0] || 'OTP verification failed';
                    setError(msg);
                    return msg;
                }
            });
            ...
            <div className="relative z-10 w-full max-w-md mx-4 animate-fade-in">
                ...
                <div className="relative glass-card p-8 rounded-2xl border border-[var(--border-color)] backdrop-blur-xl bg-[var(--card-bg)]">
                    ...
                    <div className="text-center mb-8">
                        <div className="inline-flex items-center justify-center w-20 h-20 rounded-2xl mb-4 animate-bounce-slow overflow-hidden shadow-lg shadow-purple-500/10 border border-white/5">
                        ...
                        <p className="text-[var(--text-dim)] mt-2">
                            {otpRequired ? 'A verification code has been sent to your email' : 'Sign in to continue your journey'}
                        </p>
                    </div>
                            ...
                            <div className="group animate-fade-in">
                                ...
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        className="w-full pl-12 pr-4 py-3.5 rounded-xl bg-[var(--bg-color)] border border-[var(--border-color)] text-[var(--text-main)] placeholder-[var(--text-dim)] focus:outline-none focus:ring-2 focus:ring-purple-500/50 focus:border-purple-500 transition-all duration-300 hover:border-purple-500/30 text-center font-bold tracking-widest text-lg"
                                        placeholder="000000"
                                        required
                                    />
                                </div>
                                <button 
                                    ...
                                    className="text-xs text-purple-400 hover:text-purple-300 mt-2 block hover:underline"
                                >
                                    &larr; Back to login
                                </button>
                            </div>
