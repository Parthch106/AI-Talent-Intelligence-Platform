                                    ...
                                    <div className="space-y-4">
                                        <div className="flex items-center justify-between px-2">
                                            ...
                                            <button 
                                                ...
                                                className="text-xs font-bold text-violet-400 hover:text-violet-300 transition-colors uppercase tracking-widest"
                                            >
                                                View Detailed Graph
                                            </button>
                                        </div>
                        ...
                        <div className="mb-6 bg-[var(--bg-muted)]/50 border border-[var(--border-color)] rounded-2xl p-3 backdrop-blur-md flex flex-wrap items-center gap-3">
                            ...
                            <CustomSelect
                                options={[
                                    ...
                                    ...[...new Set(tasks.map(t => t.project?.name).filter(Boolean))].map(p => ({ value: p || '', label: p || '' }))
                                ]}
                                value={projectFilter}
                                ...
                                <div className="relative z-10">
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <div className="space-y-4 relative mt-2">
                                            ...
                                            `}</style>
                                            {[
                                                { title: 'Performance Metrics', indices: [0, 1, 3], color: 'from-blue-600 to-cyan-400', headerColor: 'text-blue-400', border: 'border-blue-500/10' },
                                                { title: 'Skills & Growth', indices: [2, 5, 6], color: 'from-emerald-500 to-teal-400', headerColor: 'text-emerald-400', border: 'border-emerald-500/10' },
                                                { title: 'Engagement & Risk', indices: [4, 7, 8, 9], color: 'from-violet-600 to-fuchsia-400', headerColor: 'text-violet-400', border: 'border-violet-500/10' }
                                            ].map((group, groupIdx) => (
                                                <div key={group.title} className="space-y-2">
                                                    <div className={`text-[9px] font-black ${group.headerColor} uppercase tracking-widest border-b ${group.border} pb-1 mb-1`}>
                                                        {group.title}
                                                    </div>
                                                    <div className="space-y-2">
                                                        {group.indices.map((i, idx) => {
                                                            const key = optimalDiff.state_keys[i];
                                                            if (!key) return null; // Safety check
                                                            return (
                                                                <div key={key} className="space-y-1">
                                                                    <div className="flex justify-between text-[10px] font-bold text-[var(--text-dim)] px-1">
                                                                        <span className="capitalize">{key.replace(/_/g, ' ').replace(' norm', '')}</span>
                                                                        <span>{Math.round(optimalDiff.state_vector[i] * 100)}%</span>
                                                                    </div>
                                                                    <div className="h-1.5 bg-[var(--bg-muted)] rounded-full overflow-hidden">
                                                                        <div 
                                                                            className={`h-full rounded-full bg-gradient-to-r ${group.color} origin-left`}
                                                                            style={{ 
                                                                                width: `${Math.round(optimalDiff.state_vector[i] * 100)}%`,
                                                                                animation: 'scaleXIn 1s cubic-bezier(0.16, 1, 0.3, 1) forwards',
                                                                                animationDelay: `${(groupIdx * 3 + idx) * 100}ms`,
                                                                                transform: 'scaleX(0)'
                                                                            }} 
                                                                        />
                                                                    </div>
                            ...
                            {activeMilestoneModal.task_details ? (
                                ...
                                <div className="p-6 text-center bg-amber-500/5 border border-dashed border-amber-500/20 rounded-xl">
                                    ...
                                    <p className="text-xs text-amber-300 font-bold uppercase tracking-widest mb-1">
                                        {isGeneratingTask ? 'Task Generating…' : 'No Task Assigned'}
                                    </p>
                                    <p className="text-[11px] text-[var(--text-muted)] mb-3">
                                        {isGeneratingTask 
                                            ? 'The AI is building a hands-on task for this milestone.' 
                                            : 'Use AI to generate a hands-on task for this milestone.'}
                                    </p>
                                    {isManagerOrAdmin && (
                                        <Button
                                        ...
                                        >
                                            ...
                                            {isGeneratingTask ? 'Generating...' : 'Generate Now'}
                                        </Button>
                                    )}
                                </div>
                            )}
