...
const TaskDetailPage: React.FC = () => {
    ...
    // Distinguishes first page load from subsequent intern/manager changes
    ...
    const fetchAllTasks = async (navigateAfter = false) => {
        try {
            ...
            // Navigate to first task when: no valid taskId OR intern was switched
            if (((!taskId || taskId === 'details') || navigateAfter) && tasksData.length > 0) {
    ...
    const getSelectedInternName = (): string => {
        if (!selectedInternId) return 'All Interns';
        const found = interns.find(i => i.id === selectedInternId);
        return found?.full_name || found?.email || 'Select Intern';
    };
    ...
    useEffect(() => {
        if (isInitialMount.current) {
            ...
            fetchAllTasks(false); // First load — only redirect if no valid taskId
        } else {
            fetchAllTasks(true);  // Intern/manager changed — always go to first task
        }
    ...
    useEffect(() => {
        if (taskId && taskId !== 'details') {
            ...
            // No valid task ID — clear loading so we don't spin forever
            setLoading(false);
        }
    // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [taskId]);
    ...
    const handleSaveBriefing = async () => {
        setUpdating(true);
        toast.promise(
            axios.patch(`/analytics/tasks/${task!.id}/update-status/`, { description: briefingDraft }),
            {
                loading: 'Saving task briefing...',
                success: () => {
                    setTask(prev => prev ? { ...prev, description: briefingDraft } : prev);
                    setEditingBriefing(false);
                    setUpdating(false);
                    return 'Task briefing updated successfully';
                },
                error: (err) => {
                    setUpdating(false);
                    return (err as Error).message || 'Failed to update task briefing';
                }
            }
        );
    };
            ...
            <div className="sticky top-0 z-30 bg-[var(--bg-muted)]/80 backdrop-blur-xl border-b border-[var(--border-color)] px-6 py-4">
                <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
                    ...
                    <div className="flex items-center gap-3 w-full md:w-auto flex-wrap justify-end">
                        ...
                        {/* Intern Selector Dropdown — admin/manager only */}
                        {isManager && interns.length > 0 && (
                            <div className="relative z-40">
                                <button
                                    onClick={() => setShowInternDropdown(!showInternDropdown)}
                                    className="flex items-center gap-2 px-3 py-2.5 bg-[var(--bg-muted)]/50 border border-[var(--border-color)] rounded-xl hover:border-purple-500/50 transition-all"
                                >
                                    <div className={`w-7 h-7 rounded-lg bg-gradient-to-br ${selectedInternId ? getGradient(getSelectedInternName()) : 'from-purple-500 to-indigo-500'} flex items-center justify-center text-white font-bold text-[10px]`}>
                                        {selectedInternId ? getInitials(interns.find(i => i.id === selectedInternId)?.full_name || null) : 'ALL'}
                                    </div>
                                    <span className="text-[var(--text-main)] text-xs font-medium truncate max-w-[110px]">{getSelectedInternName()}</span>
                                    <ChevronDown size={14} className={`text-[var(--text-dim)] transition-transform ${showInternDropdown ? 'rotate-180' : ''}`} />
                                </button>

                                {showInternDropdown && (
                                    <div className="absolute top-full right-0 mt-2 w-64 bg-[var(--bg-muted)]/95 backdrop-blur-xl border border-[var(--border-color)] rounded-xl shadow-2xl z-[9999] animate-scale-in max-h-[360px] overflow-y-auto custom-scrollbar">
                                        <div className="p-2">
                                            {/* Search */}
                                            <div className="relative mb-2">
                                                ...
                                                <input
                                                    ...
                                                    className="w-full pl-9 pr-3 py-2 bg-[var(--bg-color)] border border-[var(--border-color)] rounded-lg text-xs text-[var(--text-main)] placeholder-[var(--text-muted)] focus:outline-none focus:border-purple-500"
                                                />
                                            </div>
                                            ...
                                            {!internSearch && (
                                                <button
                                                    onClick={() => { setSelectedInternId(null); setShowInternDropdown(false); setInternSearch(''); }}
                                                    className={`w-full flex items-center gap-3 p-2 rounded-lg transition-colors ${!selectedInternId ? 'bg-purple-500/20' : 'hover:bg-[var(--bg-muted)]'}`}
                                                >
                                                    <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-purple-500 to-indigo-500 flex items-center justify-center text-white font-bold text-[10px]">ALL</div>
                                                    <div className="text-left">
                                                        <p className="text-[var(--text-main)] text-xs font-bold">All Interns</p>
                                                        <p className="text-[10px] text-[var(--text-muted)]">View all tasks</p>
                                                    </div>
                                                    {!selectedInternId && <Check size={14} className="text-purple-400 ml-auto" />}
                                                </button>
                                            )}
                                            ...
                                            {filteredInterns.map((intern) => (
                                                <button
                                                    key={intern.id}
                                                    onClick={() => { setSelectedInternId(intern.id); setShowInternDropdown(false); setInternSearch(''); }}
                                                    className={`w-full flex items-center gap-3 p-2 rounded-lg transition-colors ${selectedInternId === intern.id ? 'bg-purple-500/20' : 'hover:bg-[var(--bg-muted)]'}`}
                                                >
                                                    <div className={`w-7 h-7 rounded-lg bg-gradient-to-br ${getGradient(intern.full_name || intern.email)} flex items-center justify-center text-white font-bold text-[10px] shrink-0`}>
                                                        {getInitials(intern.full_name)}
                                                    </div>
                                                    <div className="text-left min-w-0">
                                                        <p className="text-[var(--text-main)] text-xs truncate">{intern.full_name || intern.email}</p>
                                                        {intern.department && <p className="text-[10px] text-[var(--text-muted)]">{intern.department}</p>}
                                                    </div>
                                                    {selectedInternId === intern.id && <Check size={14} className="text-purple-400 ml-auto shrink-0" />}
                                                </button>
                                            ))}
                                        </div>
                                    </div>
                    ...
                    <div className="xl:col-span-2 space-y-10">
                        ...
                        <Card className="p-10">
                            <div className="flex items-center justify-between mb-6">
                                ...
                                {isManager && !editingBriefing && (
                                    <button
                                        onClick={() => { setBriefingDraft(task.description || ''); setEditingBriefing(true); }}
                                        className="flex items-center gap-2 px-3 py-1.5 text-[10px] font-black uppercase tracking-widest text-blue-400 border border-blue-500/30 rounded-lg hover:bg-blue-500/10 transition-all"
                                    >
                                ...
                                {isManager && editingBriefing && (
                                    <div className="flex gap-2">
                                        <button
                                            ...
                                            className="px-3 py-1.5 text-[10px] font-black uppercase tracking-widest text-[var(--text-dim)] border border-[var(--border-color)] rounded-lg hover:bg-[var(--bg-muted)] transition-all"
                                        >
                                            Cancel
                                        </button>
                                        <button
                                            ...
                                            disabled={updating}
                                            className="px-3 py-1.5 text-[10px] font-black uppercase tracking-widest text-white bg-blue-600 rounded-lg hover:bg-blue-500 transition-all disabled:opacity-50"
                                        >
                            ...
                            {editingBriefing ? (
                                <textarea
                                    ...
                                    className="w-full bg-[var(--bg-muted)] border border-blue-500/40 rounded-2xl px-6 py-4 text-sm text-[var(--text-main)] focus:border-blue-500/80 outline-none transition-all resize-none placeholder-[var(--text-muted)] leading-relaxed"
                                ...
                                <div className="prose prose-invert max-w-none">
                                    {(() => {
                                        ...
                                        const scriptMatch = task.description.match(/(.*?)(?:Starter Script:\s*```(?:\w*\n)?([\s\S]*?)```)(.*)/is);
                                        
                                        if (scriptMatch) {
                                            const preText = scriptMatch[1];
                                            const scriptCode = scriptMatch[2];
                                            const postText = scriptMatch[3];
                                            
                                            return (
                                                <div className="space-y-6">
                                                    <p className="text-[var(--text-main)] text-lg leading-[1.8] font-medium whitespace-pre-wrap opacity-90">
                                                        {(preText + postText).trim()}
                                                    </p>
                                                    <div>
                                                        <h4 className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-widest mb-2 px-1 flex items-center gap-2">
                                                            <Sparkles size={12} className="text-indigo-400" />
                                                            Starter Script
                                                        </h4>
                                                        <div className="relative group">
                                                            <pre className="p-4 bg-slate-950 rounded-xl text-xs text-indigo-300 font-mono overflow-x-auto border border-slate-800 custom-scrollbar shadow-inner">
                                                                <code>{scriptCode.trim()}</code>
                                                            </pre>
                                                        </div>
                                                    </div>
                                                </div>
                        ...
                        <Card className="p-8 space-y-8 relative overflow-hidden group">
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="flex items-center justify-between p-4 bg-[var(--bg-muted)] rounded-2xl border border-[var(--border-color)]">
                                    ...
                                    <span className="font-black text-[var(--text-main)] font-mono text-xs">{Math.round(task!.estimated_hours)} Hours</span>
                                </div>
                            </div>
