...
const Header: React.FC<HeaderProps> = ({ isExpanded }) => {
    ...
    return (
        <>
            <header 
                className={`h-20 bg-[var(--header-bg)] backdrop-blur-3xl border-b border-[var(--border-color)] flex items-center justify-between px-8 fixed top-0 right-0 z-30 transition-all duration-500 ease-[cubic-bezier(0.4,0,0.2,1)] ${isExpanded ? 'left-64' : 'left-20'}`}
            >
                {/* Left Section - Welcome */}
                <div className="flex items-center gap-6">
                    <div className="flex items-center gap-4">
                        {/* Removed redundant logo from header since it exists in the Sidebar */}
                        <div>
