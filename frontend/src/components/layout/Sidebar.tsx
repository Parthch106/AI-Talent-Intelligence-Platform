...
const Sidebar: React.FC<SidebarProps> = React.memo(({ isExpanded, setIsExpanded }) => {
    ...
    // Feature flag: set VITE_SHOW_V2_PAGES=true in .env to show V2 sidebar sections
    const showV2Pages = import.meta.env.VITE_SHOW_V2_PAGES === 'true';
    ...
    const adminNavItems: NavItem[] = [
        { name: 'Admin Management', path: '/management/admins', icon: <Shield size={20} />, roles: ['ADMIN'] },
    ...
    const renderNavItem = (item: NavItem, section: 'main' | 'admin' | 'monitoring' | 'intern') => {
        ...
        return (
            <li key={item.path} className="relative mb-1">
                <Link
                    to={item.path}
                ...
                >
                    ...
                    <span className={`relative z-10 text-sm font-bold tracking-tight transition-all duration-500 whitespace-nowrap overflow-hidden ${isExpanded ? 'max-w-[200px] opacity-100 ml-2' : 'max-w-0 opacity-0 ml-0'} ${active ? 'text-[var(--text-main)]' : 'text-slate-500 group-hover:text-slate-200'}`}>
                        {item.name}
                    </span>
    ...
    return (
        ...
        >
            {/* Logo Section */}
            <div className={`border-b border-[var(--border-color)] transition-all duration-500 py-4 overflow-hidden ${isExpanded ? 'px-6' : 'px-5'}`}>
                <Link to="/" className="flex items-center group">
                    <div className="relative flex-shrink-0">
                        ...
                        <div className="relative w-10 h-10 rounded-xl flex items-center justify-center shadow-2xl group-hover:scale-110 group-hover:rotate-3 transition-all duration-500 overflow-hidden border border-white/5 bg-[var(--sidebar-bg)]">
                            <img src="/aims_logo.png" alt="AIMs Logo" className="w-full h-full object-cover" />
                        </div>
                    </div>
                    <div className={`transition-all duration-500 overflow-hidden whitespace-nowrap ${isExpanded ? 'max-w-[150px] opacity-100 ml-4' : 'max-w-0 opacity-0 ml-0'}`}>
            ...
            <nav className="flex-1 overflow-y-auto py-4 px-3 sidebar-nav">
                ...
                {/* Career Management Navigation (V2) — hidden unless VITE_SHOW_V2_PAGES=true */}
                {showV2Pages && filterByRole(careerNavItems).length > 0 && (
                    <div className="mb-4">
                        <div className={`px-4 mb-2 text-[9px] font-black text-slate-600 uppercase tracking-[0.25em] flex items-center gap-3 transition-all duration-500 overflow-hidden whitespace-nowrap ${isExpanded ? 'opacity-100 max-h-8' : 'opacity-0 max-h-0'}`}>
                ...
                {/* Intern Career Navigation (V2) — hidden unless VITE_SHOW_V2_PAGES=true */}
                {showV2Pages && filterByRole(internCareerNavItems).length > 0 && (
                    <div className="mb-4">
                        <div className={`px-4 mb-2 text-[9px] font-black text-slate-600 uppercase tracking-[0.25em] flex items-center gap-3 transition-all duration-500 overflow-hidden whitespace-nowrap ${isExpanded ? 'opacity-100 max-h-8' : 'opacity-0 max-h-0'}`}>
