          ...
          {weeks.map((week, weekIdx) => (
            <div key={weekIdx} className="flex flex-col gap-[2px]">
              {week.map((day, dayIdx) => (
                <div
                  key={dayIdx}
                  className="w-3 h-3 rounded-[2px] cursor-pointer transition-transform duration-150 hover:scale-125 hover:z-10"
                  style={{ 
                    backgroundColor: getColor(day.value, day.isCurrentYear),
                    border: day.isCurrentYear ? '1px solid rgba(51, 65, 85, 0.5)' : '1px solid transparent',
