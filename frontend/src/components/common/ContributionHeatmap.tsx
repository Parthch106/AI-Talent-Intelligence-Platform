...
const ContributionHeatmap: React.FC<HeatmapProps> = ({
  ...
  // Measure container width with ResizeObserver, debounced to prevent animation lag
  useLayoutEffect(() => {
  ...
  return (
    <div className="relative p-3 bg-[var(--card-bg)] rounded-lg border border-[var(--border-color)]">
            ...
            {calendarData.map((week, weekIdx) => (
              <div key={weekIdx} style={{ display: 'flex', flexDirection: 'column', gap: `${gap}px` }}>
                {week.map((day, dayIdx) => (
                  <div
                    key={dayIdx}
                    className="rounded-[2px] cursor-pointer transition-transform duration-150 hover:scale-125"
                    style={{ 
                      width: `${cellSize}px`,
                      height: `${cellSize}px`,
      ...
      {tooltip && createPortal(
        <div
          className="fixed z-[9999] bg-[var(--card-bg)] text-[var(--text-main)] text-xs px-3 py-2 rounded-lg shadow-xl border border-[var(--border-color)] pointer-events-none animate-tooltip-fade-in"
          style={{
            left: tooltip.x + 15,
            top: tooltip.y - 15,
