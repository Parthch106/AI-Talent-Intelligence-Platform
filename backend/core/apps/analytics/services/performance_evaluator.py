...
class PerformanceEvaluator:
    ...
    def get_performance_metrics(self, days: int = 30, all_time: bool = False) -> PerformanceMetrics:
        ...
        if growth_velocity == 0.0:
            ...
            if all_completed > 0:
                # For new interns (< 5 tasks), be more lenient with growth proxy
                if all_completed < 5:
                    growth_velocity = max(0.4, all_completed / 10.0) # Start with base 0.4 if they are completing tasks
                else:
    ...
    def generate_suggestions(self, diagnosis: PerformanceDiagnosis, 
        ...
        for weak_area in diagnosis.weak_areas:
            if weak_area == "Low Task Completion Rate":
                suggestions.extend([
                    "Optimize time management with pomodoro-style task blocks",
                    "Break down complex tasks into manageable sub-tasks",
                    "Implement pair programming with senior interns for support",
                    "Set stricter daily milestones to maintain momentum",
                    "Seek immediate clarification on ambiguous task requirements"
                ])
            
            elif weak_area == "Low Code Quality":
                suggestions.extend([
                    "Enforce strict code review feedback loops before submission",
                    "Participate in refactoring workshops to improve maintainability",
                    "Apply the DRY and SOLID principles to current projects",
                    "Schedule regular 1-on-1 code review sessions with seniors",
                    "Incorporate automated linting and formatting into workflow"
                ])
            
            elif weak_area == "Low Engagement":
                suggestions.extend([
                    "Rotate project domains to explore new interest areas",
                    "Participate in team-wide collaborative syncs and standups",
                    "Introduce milestone-based rewards for personal achievements",
                    "Sync with mentors to align current work with career goals",
                    "Seek out projects with cross-departmental impact"
                ])
            
            elif weak_area == "Low Growth Velocity":
                # Only offer these if not already a top performer
                if metrics.quality_score < 0.7:
                    suggestions.extend([
                        "Increase task diversity to broaden technical exposure",
                        "Assign SKILL_GAP tasks to address core weaknesses",
                        "Leverage concept-based mini projects for skill-up",
                        "Focus on mastering one new technology per module",
                        "Engage in technical documentation to solidify learnings"
                    ])
            
            elif weak_area == "Under-challenging Tasks":
                suggestions.extend([
                    "Progressively tackle more complex architecture modules",
                    "Propose stretch goal enhancements for current project",
                    "Incorporate exploratory R&D tasks into task list",
                    "Focus on performance optimization in high-traffic code"
                ])
            
            elif weak_area == "High Dropout Risk":
                suggestions.extend([
                    "Initiate immediate priority-1 mentor check-in",
                    "Calibrate workload based on recent performance logs",
                    "Establish a short-term success plan with quick wins",
                    "Identify and remove blockers in current task assignment",
                    "Consider temporary project rotation to re-energize"
                ])
        
        # Add positive reinforcement for high performers (Thriving/Coping Well)
        performance_score = self.calculate_performance_score(metrics)
        if performance_score >= self.SCORE_THRESHOLDS['thriving']:
            suggestions.extend([
                "Lead a technical session or knowledge-sharing workshop",
                "Propose an architectural improvement for the current project",
                "Mentor junior interns on code quality and best practices",
                "Contribute to internal tool development or automation",
                "Research innovative technologies to solve current bottlenecks",
                "Act as a code quality gatekeeper for module reviews"
            ])
        elif performance_score >= self.SCORE_THRESHOLDS['coping']:
            suggestions.extend([
                "Target tasks with 4+ stars difficulty to push limits",
                "Explore cross-team collaboration for wider impact",
                "Document complex logic to improve project maintainability",
                "Increase ownership of entire project feature modules"
            ])
        
        # Deduplicate suggestions
        suggestions = list(set(suggestions))
        
        # Randomly shuffle to provide variety in different evaluations
        import random
        random.shuffle(suggestions)
        
        # Return only a focused set of top 3 suggestions
