...
class FeedbackViewSet(viewsets.ModelViewSet):
    ...
    def _create_feedback_notification(self, feedback):
        ...
        if feedback.task_status == 'COMPLETED_APPROVED':
            title = "Task Approved! 🎉"
            task_name = f"task '{feedback.task.title}'" if feedback.task else "your task"
            message = f"Your work on {task_name} has been approved! {feedback.comments[:100] if feedback.comments else ''}"
        elif feedback.task_status == 'COMPLETED_REWORK':
            title = "Feedback: Needs Rework"
            task_name = f"task '{feedback.task.title}'" if feedback.task else "your task"
            message = f"Feedback received for {task_name}. Please review the comments. {feedback.comments[:100] if feedback.comments else ''}"
        else:
