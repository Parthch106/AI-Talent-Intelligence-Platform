...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        interns_data = [
            ...
            },
            {
                ...
                'department': 'Development (Web/Application)',
                'password': 'intern123',
                'profile': {
        ...
        try:
            ...
            manager_ai.department = 'AI/ML Department'
            manager_ai.save()
            manager_web.department = 'Development (Web/Application)'
            manager_web.save()
        ...
        managers_map = {
            'AI/ML Department': manager_ai,
            'Development (Web/Application)': manager_web,
        }
        ...
        for intern_data in interns_data:
            ...
            if existing_user and force:
                # Use existing user but still create profile/data and update details
                user = existing_user
                user.department = intern_data['department']
                user.full_name = intern_data['full_name']
                user.save()
                self.stdout.write(f"Using existing user: {user.email} (updated department/name)")
            else:
                # Create new user
        ...
        self.stdout.write(self.style.SUCCESS('Intern 1: alex.johnson@intern.com / intern123 (AI/ML Department - manager@test.com)'))
        self.stdout.write(self.style.SUCCESS('Intern 2: emily.davis@intern.com / intern123 (Development (Web/Application) - manager2@test.com)'))
    ...
    def _generate_tasks(self, user, manager):
        ...
        if user.department == 'AI/ML Department':
            tasks = [
        ...
        else:  # Development (Web/Application)
            tasks = [
