            ...
            if str(user.role) == str(User.Role.MANAGER):
                ...
                if managed_depts:
                    ...
                    for dept in managed_depts:
                        q_objects |= Q(metadata__contains={'department': dept})
                    
                    activities = activities | SystemActivity.objects.filter(
                        Q(created_at__gte=start_date) & q_objects
                    )
    ...
    def get(self, request):
        ...
        if user.role == 'INTERN':
            ...
            if user.role == 'MANAGER':
                managed_depts = [d.strip() for d in user.department.split(',') if d.strip()]
                dept_overdue = dept_overdue.filter(intern__department__in=managed_depts)
        ...
        if user.role in ['ADMIN', 'MANAGER']:
            ...
            if user.role == 'MANAGER':
                managed_depts = [d.strip() for d in user.department.split(',') if d.strip()]
                recent_reports = recent_reports.filter(intern__department__in=managed_depts)
        ...
        if user.role == 'INTERN':
            ...
            managed_depts = [d.strip() for d in user.department.split(',') if d.strip()]
            risk_metrics = risk_metrics.filter(intern__department__in=managed_depts)
