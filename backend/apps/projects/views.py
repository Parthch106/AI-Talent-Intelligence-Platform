...
class ProjectViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        user = self.request.user
        if user.role == "MANAGER" and not getattr(user, 'can_create_project', True):
...
class ProjectAssignmentViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        user = self.request.user
        if user.role == "MANAGER" and not getattr(user, 'can_assign_tasks', True):
            from rest_framework.exceptions import PermissionDenied
            raise PermissionDenied("You do not have permission to assign projects/tasks.")
        serializer.save()
