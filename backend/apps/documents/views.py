...
class DocumentViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.role == "INTERN":
            ...
            # 2. Documents owned by interns in the same departments
            managed_depts = [d.strip() for d in user.department.split(',') if d.strip()]
            return Document.objects.filter(
                Q(owner=user) | 
                Q(owner__department__in=managed_depts, owner__role='INTERN')
            ).distinct()
        
        else:
            # Admin can see all documents
            # Or documents from their departments
            if user.department:
                admin_depts = [d.strip() for d in user.department.split(',') if d.strip()]
                return Document.objects.filter(
                    Q(owner__department__in=admin_depts) |
                    Q(uploaded_by__department__in=admin_depts)
                ).distinct()
...
class ResumeDataViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.role == "INTERN":
            ...
            # 2. Resume data of interns in the same departments
            managed_depts = [d.strip() for d in user.department.split(',') if d.strip()]
            return ResumeData.objects.filter(
                Q(user=user) |
                Q(user__department__in=managed_depts, user__role='INTERN')
            ).distinct()
        
        else:
            # Admin can see all resume data
            # Or resume data from their departments
            if user.department:
                admin_depts = [d.strip() for d in user.department.split(',') if d.strip()]
                return ResumeData.objects.filter(
                    Q(user__department__in=admin_depts)
                ).distinct()
