...
@shared_task(name='apps.accounts.tasks.send_email_async')
def send_email_async(receivers, subject, template_name, context):
    """
    Asynchronously renders and sends an email.
    """
    ...
    try:
        ...
        logger.info(f"Async email sent successfully to {receivers} with subject: {subject}")
    except Exception as e:
        logger.error(f"Async Email Error sending to {receivers}: {str(e)}")
