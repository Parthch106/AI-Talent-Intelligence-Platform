...
class DepartmentListView(APIView):
    ...
    def get(self, request):
        raw_departments = User.objects.exclude(department='').values_list('department', flat=True).distinct()
        departments = set()
        for dept_str in raw_departments:
            for d in dept_str.split(','):
                if d.strip():
    ...
    def get(self, request):
        ...
        if user.role == User.Role.MANAGER:
            if role_filter and role_filter != 'INTERN':
                ...
                # Manager requesting interns, filter by their departments
                managed_depts = [d.strip() for d in user.department.split(',') if d.strip()]
                users = users.filter(role=User.Role.INTERN, department__in=managed_depts)
        ...
        if user.role == User.Role.INTERN:
            ...
            if role_filter and role_filter != 'INTERN':
                ...
                users = users.filter(role=User.Role.INTERN, department__in=[d.strip() for d in user.department.split(',') if d.strip()])
...
class ManagerManagementViewSet(viewsets.ModelViewSet):
    """
    CRUD for MANAGER roles (labelled as 'Admins' in UI).
...
class BulkUploadAdminsView(APIView):
    """API endpoint to bulk upload admins (managers) via CSV"""
    permission_classes = [IsAuthenticated, IsAdmin]
    ...
    def post(self, request):
        ...
        try:
            ...
            for row in reader:
                ...
                can_create_project_raw = str(row.get('can_create_project', 'true')).strip().lower()
                can_assign_tasks_raw = str(row.get('can_assign_tasks', 'true')).strip().lower()
                
                can_create_project = can_create_project_raw in ['true', '1', 'yes']
                can_assign_tasks = can_assign_tasks_raw in ['true', '1', 'yes']
                ...
                try:
                    dispatch_email(
                        ...
                        subject=f"Welcome to {settings.SITE_NAME} - Admin Account Created",
            ...
            return Response({
                'message': f'Successfully created {created_count} admins.',
                'errors': errors
            }, status=status.HTTP_201_CREATED)
