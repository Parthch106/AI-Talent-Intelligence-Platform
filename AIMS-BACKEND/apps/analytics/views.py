...
def validate_manager_access(user, target_intern_id):
    ...
    if user.role == User.Role.MANAGER:
        try:
            ...
            if target_intern.department not in managed_depts:
...
class TaskTrackingView(APIView):
    ...
    def get(self, request, task_id=None):
        ...
        if intern_id:
            ...
            if user.role == User.Role.ADMIN:
                ...
                managed_depts = [d.strip() for d in user.department.split(',') if d.strip()]
                tasks = TaskTracking.objects.filter(intern__department__in=managed_depts)
            else:
                tasks = TaskTracking.objects.filter(intern_id=user.id)
    ...
    def post(self, request):
        ...
        if user.role not in [User.Role.MANAGER, User.Role.ADMIN]:
            return Response(
                {'error': 'Permission denied. Only Admins and Super Admins can create tasks.'},
                status=status.HTTP_403_FORBIDDEN
            )
    ...
    def patch(self, request, task_id=None):
        """Update task — details (title/description/priority/due_date) for ADMIN+MANAGER, status for MANAGER+INTERN."""
        user = request.user
        ...
        # ── MANAGER: status + details, dept-scoped ──
        if user.role == User.Role.MANAGER:
        ...
        if new_status:
            ...
            if old_status != 'COMPLETED' and new_status == 'COMPLETED':
                from apps.analytics.services.learning_path_optimizer import process_task_completion_for_skills
                process_task_completion_for_skills(task)
    ...
    def delete(self, request, task_id=None):
        ...
        if user.role not in [User.Role.MANAGER, User.Role.ADMIN]:
            return Response(
                {'error': 'Permission denied. Only Admins and Super Admins can delete tasks.'},
                status=status.HTTP_403_FORBIDDEN
            )
...
class TaskEvaluationView(APIView):
    ...
    def patch(self, request, task_id=None):
        ...
        if user.role not in [User.Role.MANAGER, User.Role.ADMIN]:
            return Response(
                {'error': 'Permission denied. Only Admins and Super Admins can evaluate tasks.'},
                status=status.HTTP_403_FORBIDDEN
            )
...
class AttendanceRecordView(APIView):
    ...
    def post(self, request):
        ...
        # Restrict interns to only mark attendance for the current day
        if user.role == User.Role.INTERN:
        ...
        working_hours_val = float(request.data.get('working_hours', 0.0))

        if check_in_time_str:
            try:
                from datetime import datetime
                # Handle possible formats: H:M:S or H:M
                ci_fmt = '%H:%M:%S' if check_in_time_str.count(':') == 2 else '%H:%M'
                ci_time = datetime.strptime(check_in_time_str, ci_fmt)
                
                # Rule 1: Late Penalty
                if ci_time.hour > 10 or (ci_time.hour == 10 and ci_time.minute > 30):
                    status_val = 'LATE'
                
                # Rule 2: Mandatory Hours (if checked out)
                if check_out_time_str:
                    co_fmt = '%H:%M:%S' if check_out_time_str.count(':') == 2 else '%H:%M'
                    co_time = datetime.strptime(check_out_time_str, co_fmt)
                    
                    diff_hours = (co_time - ci_time).total_seconds() / 3600.0
                    working_hours_val = round(diff_hours, 2)
                    
                    if diff_hours < 8.0:
                        status_val = 'HALF_DAY'
            except Exception as e:
                logger.error(f"Error parsing attendance times: {e}")
        ...
        attendance, created = AttendanceRecord.objects.update_or_create(
            ...
            defaults={
                ...
                'working_hours': working_hours_val,
...
class WeeklyReportView(APIView):
    ...
    def post(self, request):
        ...
        is_draft = str(report_data.get('is_draft', 'false')).lower() == 'true'
    ...
    def patch(self, request):
        """Update a weekly report (e.g., finalize draft)."""
        user = request.user
        ...
        if user.role == User.Role.INTERN and report.intern_id != user.id:
            return Response(
                {'error': 'Permission denied: Cannot update other users reports'},
                status=status.HTTP_403_FORBIDDEN
            )
        ...
        if is_submitted is not None:
            is_submitted_bool = str(is_submitted).lower() == 'true'
            report.is_submitted = is_submitted_bool
            if is_submitted_bool and not report.submitted_at:
                report.submitted_at = timezone.now()
            elif not is_submitted_bool:
...
class DropoutRiskDashboardView(APIView):
    ...
    def get(self, request):
        ...
        if user.role == User.Role.MANAGER:
            interns = User.objects.filter(role=User.Role.INTERN, department__in=[d.strip() for d in user.department.split(',') if d.strip()])
        else:
...
class LearningPathView(APIView):
    ...
    def post(self, request, intern_id):
        ...
        if path_type != 'skill' and not target_role:
...
class LearningPathProgressView(APIView):
    ...
    def get(self, request, intern_id):
        ...
        try:
            ...
            if progress is None:
                return Response({'exists': False, 'message': 'No learning path found.'}, status=status.HTTP_200_OK)
...
class LLMTaskSuggestionView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            # Get intern's skills from SkillProfile
            from apps.analytics.models import SkillProfile
            skill_profiles = SkillProfile.objects.filter(intern_id=intern_id)
            if skill_profiles.exists():
                skills = [f"{sp.skill_name} ({sp.mastery_level*100:.0f}% mastery)" for sp in skill_profiles]
            else:
            ...
            # Get optimal difficulty from RL Agent
            from apps.analytics.services.rl_task_assigner import get_optimal_difficulty
            optimal_difficulty = get_optimal_difficulty(intern_id)
            ...
            return Response({
                ...
                'completed_tasks': completed_tasks,
                'ongoing_tasks': ongoing_tasks,
                'message': f'Generated {len(result.get("pathways", []))} task pathways'
            }, status=status.HTTP_200_OK)
...
class EmploymentStageViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if str(user.role) == 'MANAGER' and user.department:
            managed_depts = [d.strip() for d in user.department.split(',') if d.strip()]
            qs = qs.filter(intern__department__in=managed_depts)
...
class PhaseEvaluationViewSet(viewsets.ModelViewSet):
    ...
    def list_eligible(self, request):
        ...
        if str(user.role) == 'MANAGER' and user.department:
            queryset = queryset.filter(user__department__in=[d.strip() for d in user.department.split(',') if d.strip()])
...
class FullTimeOfferViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.role == 'MANAGER' and user.department:
            managed_depts = [d.strip() for d in user.department.split(',') if d.strip()]
            queryset = queryset.filter(intern__department__in=managed_depts)
        return queryset
