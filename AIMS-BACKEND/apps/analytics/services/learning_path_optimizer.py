...
def _auto_generate_tasks_for_path(learning_path, intern_name: str, goal_text: str, basics_only: bool = False) -> None:
    ...
    if changed:
        learning_path.milestones = list(milestones)
...
def save_milestone_task(intern_id: int, milestone_index: int, task_data: dict) -> bool:
    ...
    if 0 <= milestone_index < len(path.milestones):
        ...
        path.milestones = list(path.milestones)
        path.save()
        return True
    return False
...
def approve_milestone_task(intern_id: int, milestone_index: int) -> dict:
    ...
    try:
        ...
        path.milestones = list(path.milestones)
        path.save(update_fields=['milestones'])
        return {
...
def get_path_progress(intern_id: int) -> Optional[dict]:
    ...
    mastery_map = {sp.skill_name.lower(): sp.mastery_level for sp in SkillProfile.objects.filter(intern_id=intern_id)}
    milestones = path.milestones
    for m in milestones:
        # Check both 'skill' and 'area' keys for the skill name
        skill_name = m.get('skill', m.get('area', '')).lower()
        if skill_name:
            m['current_mastery'] = round(mastery_map.get(skill_name, m.get('current_mastery', 0.0)), 2)
            
        if m.get('current_mastery', 0.0) >= 0.8:
            m['status'] = 'COMPLETED'
        elif m.get('current_mastery', 0.0) > 0:
...
def process_task_completion_for_skills(task) -> None:
    """
    Process a completed task to update the intern's skill proficiency.
    Extracts skills from task.skills_required or infers them from title/description.
    """
    from apps.analytics.models import SkillProfile
    import re
    
    skills = set(task.skills_required) if getattr(task, 'skills_required', None) else set()
    
    # If no skills attached, infer from title and description
    if not skills:
        text = f"{task.title} {task.description}".lower()
        known_skills = list(PREREQUISITE_GRAPH.keys())
        for ks in known_skills:
            ks_lower = ks.lower()
            # Simple word boundary check or exact substring for special symbols
    ...
    # Determine mastery increment
    base_increment = 0.1
    if getattr(task, 'quality_rating', None):
        # Scale increment based on quality rating (0-5)
        base_increment = 0.1 * (task.quality_rating / 5.0)
    ...
    for skill in skills:
        ...
        new_mastery = profile.mastery_level + base_increment
        update_progress(task.intern_id, skill, new_mastery)
