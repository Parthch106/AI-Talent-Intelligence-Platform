...
def _calculate_skill_mastery(task):
    ...
    """
    try:
        # Allow processing for all tasks that have skills defined
        # Even if they don't have a project_assignment
        ...
        for skill_name in skills:
            ...
            if existing_profile:
                # Update existing profile mastery
                old_mastery = existing_profile.mastery_level
                existing_profile.mastery_level = min(1.0, old_mastery + mastery_level)
                existing_profile.save(update_fields=['mastery_level', 'last_updated'])
                skills_updated.append(skill_name)
                logger.info(f"Updated SkillProfile for {task.intern.email}: {skill_name} = {old_mastery:.2f} -> {existing_profile.mastery_level:.2f}")
            else:
